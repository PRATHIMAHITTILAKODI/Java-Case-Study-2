package com.gl.cafe.app;
import java.io.*;
import java.text.DateFormat;
import java.text.*;
import java.util.*;
import com.gl.cafe.app.OrderedItem;
public class CafeMain {
	...
	public static String salesTaxCalculation(double billAmount) {
		  double taxAmount = 0.0;
	        ...
	        } else if (billAmount > 1500 && billAmount <= 2500) {
	            taxAmount = billAmount * 0.15;
	        } else {
	...
	public static void main(String[] args) {
		...
		for (int i=0;i<n;i++) {
			...
			if (ItemData.isAvailable(iname)) {
				...
				if (Double.parseDouble(iqty)>=Double.parseDouble(discountQty)) {
					...
					discountAmount=payAmount * (Double.parseDouble(discountRate)/150);
		...
		String output=String.format("%-15s %-15s %-15s %-15s %-15s %-15s", "ITEM" ,"RATE","QUANTITY","PRICE","DISCOUNT","AMOUNT");
		...
		for (OrderedItem i:orderedItemList) {
			...
			String output1=String.format("%-15s %-15s %-15s %-15s %-15s %-15s", name,r,q,price,dis,amount);
